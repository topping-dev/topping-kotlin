...
actual open class LuaViewModelProvider : KTInterface {
    ...
    actual companion object {
        actual fun of(fragment: LuaFragment): LuaViewModelProvider {
            ...
            val nat =
                cocoapods.Topping.LuaViewModelProvider.ofFragment(fragment.GetNativeObject() as cocoapods.Topping.LuaFragment)
        ...
        actual fun of(form: LuaForm): LuaViewModelProvider {
            val prov = LuaViewModelProvider()
            val nat =
                cocoapods.Topping.LuaViewModelProvider.ofForm(form.GetNativeObject() as cocoapods.Topping.LuaForm)
            prov.SetNativeObject(nat)
            return prov
        }
    }
    ...
    actual inline fun <reified T : Any> get(key: String, obj: T): T {
        return (luaViewModelProvider?.get(
            key, StableRef.create(obj)
                .asCPointer()
        ) as COpaquePointer).asStableRef<T>().get()
    }
